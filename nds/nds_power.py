...
def gen_sql_from_stream(query_file_path, run_single):
    """Read Spark compatible query stream and split them one by one

    Args:
        query_file_path (str): path of query file or query stream file generated by TPC-DS tool
    ...
    with open(query_file_path, 'r') as f:
        query = f.read().split(";")[0] # pick the first query in the file
        query_name = os.path.basename(query_file_path)
        return dict([(query_name, query)])
...
def run_query_stream(input_prefix,
    ...
    if sub_queries and not run_single:
...
if __name__ == "__main__":
    ...
    parser.add_argument('query_file',
                        help='query file or query stream file that contains NDS queries in specific order')
